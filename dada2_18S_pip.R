...
FWD <- c("YRYRGGCTTAATTTGACTCAACRCG")

REV <- c("GGGCATCACAGACCTGTTAT")
...
sample_names <- sapply(strsplit(basename(fas_Fs_process), "_"), function(x) {paste0(x[c(1,5)],collapse="_")} )
...
out_2 <- filterAndTrim(fas_Fs_process, fas_Fs_filtered, fas_Rs_process, fas_Rs_filtered,
                       truncLen = c(200,200), maxN = 0, maxEE = c(2, 2), truncQ = 2,
...
# and classifying
tax <- IdTaxa(test=dna, trainingSet=trainingSet, strand="both", processors=NULL, verbose=FALSE)
...
write.csv(asv_tax, file.path(path_results, "seq_nochim_tax.csv"), row.names = TRUE)
...
write.csv(asv_tab_no_contam, file.path(path_results, "ASVs_counts-no-contam.csv"), row.names = TRUE)
write.csv(asv_tax_no_contam, file.path(path_results, "ASVs_taxonomy-no-contam.csv"), row.names = TRUE)
